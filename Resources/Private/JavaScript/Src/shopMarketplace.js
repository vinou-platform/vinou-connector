...
var vinouShop = {
	...
	// 	else
		...
		var quantities = document.querySelectorAll('*[data-basket-quantity]');
		for (var i = 0; i < quantities.length; i++){
			ctrl.sum.quantity = quantities[i].getAttribute('data-basket-quantity');
			quantities[i].remove();
	...
	addItemToBasket: function(item) {
		...
		ctrl.basketAction('addItemWithInit',postData,(function(){
			if (this.status === 200) {
	...
	// 	ctrl.sum.gross = 0;
	// 	ctrl.sum.quantity = 0;
		ctrl.basketAction('getBasketSummary',{},(function(){
			if (this.status == 200) {
				response = JSON.parse(this.responseText);

				if (response.summary !== false)
					if (response.summary.quantity) {
						ctrl.sum.quantity = response.summary.quantity;
						ctrl.updateBasketSum();
					}
					// // ToDo: quantity mit einziehen
					// if (response.quantity) {
					// 	ctrl.sum.quantity = response.quantity;
					// 	ctrl.updateBasketSum();
					// }
	...
	var quantitySelects = document.querySelectorAll(t3vinprefix + '[name="quantity"]');
		...
		basketRefresh = document.querySelector(t3vinprefix + 'a.button-to-refresh');
		if (basketRefresh)
			basketRefresh.addEventListener('click',function(event) {
				event.preventDefault();
				var inputBasketAction = document.querySelectorAll(t3vinprefix + '.inputBasketAction');
				inputBasketAction[0].value = 'refreshBasket';
				ctrl.disableForm(this.closest('form'));
				this.closest('form').submit();
			});

		basketCheckout = document.querySelector(t3vinprefix + 'a.button-to-checkout');
		if (basketCheckout)
			basketCheckout.addEventListener('click',function(event) {
				event.preventDefault();
				var inputBasketAction = document.querySelectorAll(t3vinprefix + '.inputBasketAction');
				inputBasketAction[0].value = 'goToCheckout';
				ctrl.disableForm(this.closest('form'));
				this.closest('form').submit();
			});
	...
	setBasketValidation: function(validation) {
		if (!validation) {
			var btn = document.querySelector(t3vinprefix + 'a.button-to-checkout');
			if (btn)
				btn.parentNode.removeChild(btn);

			var btn = document.querySelector(t3vinprefix + 'a.button-to-refresh');
				if (btn){
